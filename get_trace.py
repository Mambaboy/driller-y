...
def dynamic_trace(tracer_qemu,input_path,target_binary,output_dir,from_dir,input_from,add_env):
        '''
...
#                  if v.startswith('Trace')] # 得到所有的基本块地址,删掉了别的内容 str类型
        ...
        write_each_trace(output_dir,input_name, addrs ,from_dir)
            
        os.remove(lname)#删除记录测试用例轨迹的临时文件
        return (addrs,crash_addr)  #返回一个list  如果是crash,addrs是包括最后一个的

def write_out_all_trace(trace_set,crash_set,output_dir): #包括crash和queue
    #输出目录
...
def write_each_trace(output_dir,input_name, test_trace ,from_dir):
...
def main(argv):
    print "start!\n"
    #配置目标程序
    target_binary = "/home/xiaosatianyu/Desktop/driller/binary-cgc/CROMU_00046" #这个可以读取 bmp2tiff claw32
    ...
    #queue_dir = os.path.join(afl_dir, "queue")  # AFL生成测试用例的目录
    queue_dir = "/home/xiaosatianyu/Desktop/driller/seed"
    
    crash_dir = os.path.join(afl_dir, "crashes")  # AFL生成测试用例的目录
    
    #配置输出目录
    target_base=os.path.basename(target_binary) #在tmp/程序名 下
    output_dir=os.path.join("/tmp",target_base)
    ...
    for input_file in queue_inputs:
        input_data_path = os.path.join(queue_dir, input_file) 
        from_dir="queue"
        addrs,crash_address=dynamic_trace(tracer_qemu,input_data_path,target_binary,output_dir,from_dir, input_from, add_env=add_env)#记录对应测试用例的轨迹
    ...
    #输出所有的轨迹到文件
    write_out_all_trace(trace_set,crash_block_set,output_dir)
